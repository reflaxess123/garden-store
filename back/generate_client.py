...
def generate_api_client(schema: Dict[str, Any]) -> str:
    ...
    client_content.append("  }")
    client_content.append("")
    client_content.append("  // Если ответ 204 No Content или тело пустое, возвращаем undefined")
    client_content.append("  if (")
    client_content.append("    response.status === 204 ||")
    client_content.append("    !response.headers.get('content-length') ||")
    client_content.append("    response.headers.get('content-length') === '0'")
    client_content.append("  ) {")
    client_content.append("    return undefined as T;")
    client_content.append("  }")
    client_content.append("")
