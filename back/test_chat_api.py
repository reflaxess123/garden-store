...
async def test_chat_api() -> None:
    ...
    login_data = {"email": "reflaxess@gmail.com", "password": "123123"}

    async with httpx.AsyncClient() as client:
        try:
            # 1. Логинимся
            print("1. Логинимся как админ...")
            login_response = await client.post(f"{base_url}/api/auth/signin", json=login_data)
            ...
            print("\n2. Создаем чат...")
            create_chat_response = await client.post(f"{base_url}/api/chats", cookies=cookies)
            ...
            admin_chats_response = await client.get(f"{base_url}/api/admin/chats", cookies=cookies)
            ...
            if chat_id:
                ...
                message_data = {"chatId": chat_id, "message": "Тестовое сообщение от админа"}
                send_message_response = await client.post(
                    f"{base_url}/api/chats/{chat_id}/messages", json=message_data, cookies=cookies
                )
